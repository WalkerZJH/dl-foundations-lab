...
from .im2col import *
from .layers import conv_backward_naive, conv_forward_naive
...
def conv_forward_im2col_indices(x, w, b, conv_param):
    """
    Pure NumPy im2col fallback for environments without the Cython extension.
    """
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    stride, pad = conv_param["stride"], conv_param["pad"]
...
def conv_backward_im2col_indices(dout, cache):
    """
    Pure NumPy col2im fallback for environments without the Cython extension.
    """
    x, w, b, conv_param, x_cols = cache
    stride, pad = conv_param["stride"], conv_param["pad"]
    F, _, HH, WW = w.shape
...
if HAS_CYTHON:
    ...
    conv_forward_fast = conv_forward_im2col_indices
    conv_backward_fast = conv_backward_im2col_indices
