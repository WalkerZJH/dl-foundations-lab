...
│   ├── data_loading.py            # Digits 加载与分层划分
...
每个正式 run 保存 `config.json`、`epoch_metrics.csv`、`run_trace.txt`、`training_summary.json` 和曲线。`training_checkpoint.pkl` 仅用于本地恢复，受 `.gitignore` 排除。
